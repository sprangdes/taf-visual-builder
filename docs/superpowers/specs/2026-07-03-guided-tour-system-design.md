...
Every tour step is explanatory and can advance without requiring an editor action. Desktop and tablet receive the complete experience; mobile receives a simplified explanatory experience. Tours never open automatically.
...
The decision is based on its React-oriented API, asynchronous step hooks, explicit lifecycle events, dynamic-target support, and custom tooltip rendering.

Rejected alternatives:

- **Driver.js:** lightweight and highly themeable, but its framework-neutral API would require more application-owned coordination for React state and dynamic targets.
...
- Keep every step passive, with Back, Next, Skip, and Close always available as appropriate.
...
- **`TourProvider`:** owns the active tour, lifecycle state, device presentation, demonstration-data snapshot, and cleanup.
- **Tour definitions:** declarative definitions for quick-start, new-feature, and topic-help tours. Each step declares a stable ID, target, copy, mobile copy, and optional lifecycle hooks.
...
- Show the step number, title, concise explanation, progress, Back, Next or completion action, and Close.
...
- Provide a concise explanation and an action to scroll to the relevant section.
...
Every step is passive. Next is never disabled by editor state, and tour definitions do not declare task-completion events. The highlighted target provides spatial context only; users may interact with the editor while a tour is open, but doing so is optional and has no effect on navigation.
...
6. Every step allows immediate forward and backward navigation.
...
- Cleanup is idempotent and removes listeners, temporary UI state, overlays, and retained snapshots.
...
- Back, Next, Skip, Close, and progress have explicit accessible names.
...
- Keep Next available on every non-final step without editor interaction.
...
- Verify copy, progress, and control labels.
- Verify no step renders task instructions, task-focus classes, or a disabled Next action.
...
5. Add demonstration-data restoration.
...
- Desktop, tablet, and mobile tours use passive instructions and never require editor interaction to advance.
